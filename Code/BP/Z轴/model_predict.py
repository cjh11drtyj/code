...
print('Z轴BP训练集:', train_data.shape, train_label.shape)
print('Z轴BP测试集:', test_data.shape, test_label.shape)
...
# 注意：这里加载的是Z轴BP模型
model = load_model('./bp_z_model_50_16.keras')
...
# 4.4 加上BP模型修正后的SGP4位置z
sgp4_z_bp = sgp4_z + predict_results

# 4.5 BP修正后的Z轴位置误差
Val_z_bp = cpf_z - sgp4_z_bp
...
# 绘制400min 原始位置z误差与BP修正后误差
plt.figure(figsize=(8, 6))
plt.plot(Val_z[:400], label='Original Error')
plt.plot(Val_z_bp[:400], label='BP Corrected Error')
plt.legend()
...
# 绘制800min 原始位置z误差与BP修正后误差
plt.figure(figsize=(8, 6))
plt.plot(Val_z[:800], label='Original Error')
plt.plot(Val_z_bp[:800], label='BP Corrected Error')
plt.legend()
...
# 绘制1440min 原始位置z误差与BP修正后误差
plt.figure(figsize=(8, 6))
plt.plot(Val_z, label='Original Error')
plt.plot(Val_z_bp, label='BP Corrected Error')
plt.legend()
...
plt.plot(Val_z_bp[:400], label='BP Corrected Error')
plt.legend()
plt.title('400 min BP Corrected Z Error')
plt.xlabel('Time / min')
plt.ylabel('Z Error')
plt.show()
...
plt.plot(Val_z_bp[:800], label='BP Corrected Error')
plt.legend()
plt.title('800 min BP Corrected Z Error')
plt.xlabel('Time / min')
plt.ylabel('Z Error')
plt.show()
...
plt.plot(Val_z_bp, label='BP Corrected Error')
plt.legend()
plt.title('1440 min BP Corrected Z Error')
plt.xlabel('Time / min')
plt.ylabel('Z Error')
plt.show()
