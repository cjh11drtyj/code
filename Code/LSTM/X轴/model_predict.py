...
for i in range(len(val_x)):
    data.append([val_x[i],  ax[i]]) # 将位置误差, 速度, 加速度整合到一起
...
model = load_model('./model_10_16.keras')  # 加载之前保存的训练好的模型
    
# ====================================================================
# 3. 模型预测
last_one = train_data[-1] # 取出训练数据集的最后一个数据, 预测得到测试集数据集第一个数据的位置误差
last_one = last_one.reshape(1, time_steps, 2) # 对last_one的shape进行变形, 满足模型的输入shape
...
for i in range(test_data.shape[0]):        # 遍历测试数据集
    ...
    temp = temp.reshape(-1, time_steps, 2) # 对temp的shape进行变形, 满足模型的输入shape
    last_one = temp
